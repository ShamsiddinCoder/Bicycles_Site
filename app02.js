...
hero_slide_control_items.forEach((items, index) => {
    items.addEventListener(`click`, () => showSlide(index));
})

// puse slide when mouse come in slider 
...
let scroll = window.requestAnimationFrame || function(callback) {window.setTimeout(callback, 1000)};
...
isElInViewPort = (el) => {
    ...
    let destance = 200;
    return (rect.top <= (window.innerHeight - destance || document.documentElement.clientHeight - destance));
}
